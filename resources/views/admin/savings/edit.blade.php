...
@section('title', 'Form Edit Simpanan')
...
@section('content')
    <div class="max-w-2xl mx-auto bg-slate-100 rounded-xl shadow-xs border border-slate-200 overflow-hidden">
        <div class="bg-slate-200 p-6 border-b border-slate-200">
            <h3 class="text-slate-900">Edit Simpanan</h3>
        ...
        <form action="{{ route('savings.update', $saving->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')

            <div>
                <label for="user_id">Nama Anggota Koperasi</label>
                <input type="text" name="user_id" value="{{ $saving->user->name }}" disabled>
            </div>
            ...
            <div>
                ...
                <div class="relative rounded-lg shadow-xs">
                    ...
                    <input class="pl-9!" name="amount" type="text" value="{{ number_format($saving->amount, 0, ',', '') }}" required autocomplete="off">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="note" rows="2" placeholder="{{ $saving->note }}"></textarea>
            </div>
